...
export namespace nyan
{
	...
	public:
			...
			m_data()
		{
			...
			Scalar qi = quaternion.get_imaginary()[0];
			Scalar qj = quaternion.get_imaginary()[1];
			Scalar qk = quaternion.get_imaginary()[2];
			...
			for (size_t y = 0; y < Size_y; y++) {
				for (size_t x = 0; x < Size_x; x++) {
					...
					if constexpr (column_major)
						at(x, y) = Scalar(list[nyan::at<Size_y, column_major>(x, y)]);
					else
						at(x, y) = Scalar(list[nyan::at<Size_x, column_major>(x, y)]);
				}
			}
		}
		template<typename T>
		constexpr explicit Mat(const std::vector<T>& vec) : m_data() {
			for (size_t y = 0; y < Size_y; y++) {
				for (size_t x = 0; x < Size_x; x++) {
					//Memory layout differs from user expectation
					if constexpr (column_major)
						at(x, y) = Scalar(vec[nyan::at<Size_y, column_major>(x, y)]);
					else
						at(x, y) = Scalar(vec[nyan::at<Size_x, column_major>(x, y)]);
				}
			}
		}
		...
		constexpr explicit Mat(Args... args) noexcept {
			...
			if constexpr (sizeof...(Args) == 1) {
				...
				for (size_t y = 0; y < Size_y; y++) {
					for (size_t x = 0; x < Size_x; x++) {
						at(x, y) = Scalar{ list[nyan::at<Size_x, false>(x, y)] };
					}
				}
			}
		}
		...
		constexpr inline Vec<Scalar, 3> euler() const noexcept
		{
			...
			if (nyan::close(col(0)[2], Scalar(-1.0f))) {
				return Vec<Scalar, 3>{ nyan::pi_2* nyan::rad_to_deg,
					nyan::pi_2* nyan::rad_to_deg,
					std::atan2(col(1)[0], -col(1)[1])* nyan::rad_to_deg };
			}
			else if (nyan::close(col(0)[2], Scalar(1.0f))) {
				return Vec<Scalar, 3>{ nyan::pi_2* nyan::rad_to_deg,
					-nyan::pi_2 * nyan::rad_to_deg,
					std::atan2(col(1)[0], -col(1)[1])* nyan::rad_to_deg };
			}
			else {
				return Vec<Scalar, 3>{ std::atan2(col(1)[2], col(2)[2])* nyan::rad_to_deg,
					std::atan2(-col(0)[2], std::sqrt(col(1)[2] * col(1)[2] + col(2)[2] * col(2)[2]))* nyan::rad_to_deg,
					std::atan2(col(0)[1], col(0)[0])* nyan::rad_to_deg };
			}
		}
		...
		*/
		...
		{
			Mat<Scalar, 4, 4, column_major> mat = Mat<Scalar, Size_y, Size_x, column_major>::affine_transformation_matrix(roll_pitch_yaw, translation_vector);
